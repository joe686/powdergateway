...
- 🔵 阶段七（平台支撑能力）**2026-07-22 立项**：
  - **FN-11 扩展**（导入导出 Excel/MD + 菜单合并）· 实施任务清单已就绪，可开工
  - **REG-1**（注册中心 Nacos+Eureka）· 实施任务清单已就绪，可开工
  - **TEST-1**（测试工具改造）· 详细开发计划待用户批准
  - **REL-1**（打包发布 · 便携版 + 标准版 + 自动 CI）· 详细开发计划待用户批准
  - 文档索引：详见 `docs/03-开发/开发计划.md` §阶段七
